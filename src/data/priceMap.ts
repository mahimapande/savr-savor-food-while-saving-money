...
const QTY_UNIT_RE =
  /^([\d]+(?:[./][\d]+)?(?:\s+[\d]+\/[\d]+)?)\s*(cups?|cans?|tbsps?|tsps?|oz|lbs?|bunch(?:es)?|cloves?|blocks?|large|small|medium|slices?|sheets?|inch|fillets?|stalks?|sticks?|jars?|bags?|box(?:es)?|cartons?|bottles?|pinch(?:es)?|dash(?:es)?|loaves|loaf|gallons?|pints?|quarts?|liters?|ml|g|kg|dozens?)\b\s*/i;
